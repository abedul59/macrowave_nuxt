        ...
        <div class="container pb-5 mt-4">
          ...
          <div class="card shadow-sm mb-4" v-show="!isChartLoading && taiexAllData">
            <div class="card-body">
              ...
              <div id="taiexChart" style="width: 100%; height: 650px;"></div>
            </div>
          </div>
...
const taiexAllData = ref(null)    // 存放所有週期資料
const currentPeriod = ref('daily') // 預設顯示日K
...
async function initTaiexChart() {
  ...
  try {
    ...
    if (lastWeeklyWave) {
      ...
      if (pointDiff >= 800) { weeklyAlert = `⚠️ 週線波段已漲 ${Math.round(pointDiff)} 點，超越 600~800 極值，隨時有回檔風險！`; wAlertClass = 'alert-danger' }
      else if (pointDiff >= 600) { weeklyAlert = `⚡ 週線達極值下緣，留意月線 (20MA) 支撐。`; wAlertClass = 'alert-warning' }
      else { weeklyAlert = `✅ 中線極值未滿足，若站穩 20MA 仍有空間。`; wAlertClass = 'alert-success' }
    }
    ...
    if (lastMonthlyWave) {
      ...
      if (pointDiff >= 2000) {
        if (monthlyKD[monthlyKD.length-1].k > 80) {
          monthlyAlert = `⚠️ 長波段漲 ${Math.round(pointDiff)} 點滿足 2000 點要求！若季線(60MA)下彎將結束長波段。`; mAlertClass = 'alert-danger'
        } else { monthlyAlert = `⚡ 進入長線高檔風險區。`; mAlertClass = 'alert-warning' }
...
// 根據 currentPeriod (daily/weekly/monthly) 重新渲染 ECharts
function renderEChart() {
  ...
  for (let i = 1; i < targetData.length; i++) {
    ...
    if (prev.k <= prev.d && curr.k > curr.d) {
      crossMarks.push({ coord: [i, curr.k], symbol: 'arrow', symbolSize: 12, itemStyle: { color: '#dc3545' }, value: '金叉' })
    }
    // 死亡交叉 (綠向下)
    if (prev.k >= prev.d && curr.k < curr.d) {
      crossMarks.push({ coord: [i, curr.k], symbol: 'arrow', symbolRotate: 180, symbolSize: 12, itemStyle: { color: '#198754' }, value: '死叉' })
    }
  }
  ...
  setTimeout(() => {
    ...
    // 設定 ECharts 雙視窗 (上方 K線 + MA，下方 KD)
    chartInstance.setOption({
      tooltip: { trigger: 'axis', axisPointer: { type: 'cross' } },
      axisPointer: { link: [{ xAxisIndex: 'all' }] }, // 上下圖表 Hover 同步
      legend: { data: ['K線', '20MA', '60MA', 'K值', 'D值'] },
      grid: [
        { left: '8%', right: '5%', top: '10%', height: '55%' }, // 主圖 (K線)
        { left: '8%', right: '5%', top: '75%', height: '20%' }  // 子圖 (KD)
      ],
      xAxis: [
      ...
      series: [
        ...
        { 
          ...
          markPoint: { data: crossMarks, label: { show: false } } // 在 KD 子圖上標示交叉點
        },
        { name: 'D值', type: 'line', data: dData, xAxisIndex: 1, yAxisIndex: 1, smooth: true, symbol: 'none', lineStyle: { color: '#0d6efd' } }
      ]
    }, true) // true 代表重新繪製，清除前一個週期的殘留線條
  }, 100)
}
</script>

<style>
/* 請保留您原本的 CSS 樣式，此處省略以保持整潔 */
</style>
