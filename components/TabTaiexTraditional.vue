<template>
  <div class="container pb-5 mt-4">
    ...
    <div v-if="isLoading" class="text-center my-5"><div class="spinner-border text-primary"></div><p>運算中...</p></div>
    ...
    try {
        ...
        analysis.value = {
            ...
            weekly: { 
                ...
                analysisTxt: lw ? `前次${lw.txt}於 ${lw.date}。變動 ${Math.round(Math.abs(latest-lw.close))} 點。` : '無',
                alertText: lw && Math.abs(latest-lw.close)>800 ? '⚠️ 超出傳統極值' : '✅ 尚有空間', alertClass: lw && Math.abs(latest-lw.close)>800?'alert-danger':'alert-success'
            },
            monthly: {
                k: taiexData.value.monthly.slice(-1)[0].k, d: taiexData.value.monthly.slice(-1)[0].d,
                analysisTxt: lm ? `前次${lm.txt}於 ${lm.date}。變動 ${Math.round(Math.abs(latest-lm.close))} 點。` : '無',
                alertText: lm && Math.abs(latest-lm.close)>2000 ? '⚠️ 長線達標' : '✅ 長線趨勢中', alertClass: lm && Math.abs(latest-lm.close)>2000?'alert-danger':'alert-success'
            }
        };
        renderChart();
